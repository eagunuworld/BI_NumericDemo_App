...
PORT=$(kubectl -n west-prod get svc ${svcName} -o json | jq .spec.ports[].nodePort)

# first run this
chmod 777 $(pwd)
echo $(id -u):$(id -g)
docker run -v $(pwd):/zap/wrk/:rw -t owasp/zap2docker-weekly zap-api-scan.py -t http://$serverURL:$PORT/v3/api-docs -f openapi -r zap_report.html
